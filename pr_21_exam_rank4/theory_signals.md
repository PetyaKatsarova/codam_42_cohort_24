...
SIGALRM = "Signal Alarm" - A timer signal sent by the operating system when an alarm() timer expires.

#define SIGALRM 14  // Signal number on Unix systems
alarm(5);  // "Send me SIGALRM in 5 seconds"
// Without handler (default):
alarm(5);
sleep(10);  // Process gets KILLED after 5 seconds!

// With your handler:
sigaction(SIGALRM, &sa, NULL);  // Install custom handler
alarm(5);
sleep(10);  // After 5 seconds: alarm_handler() runs, then continues

Why SIGALRM is Perfect for Timeouts ⏱️
The Problem:
waitpid() blocks indefinitely waiting for child
Child might hang forever (infinite loops, deadlocks)
Need to interrupt waitpid() after a timeout
The Solution:
SIGALRM interrupts blocking system calls
When SIGALRM arrives, waitpid() returns -1
errno becomes EINTR (interrupted by signal)
Now you can detect the timeout and handle it!