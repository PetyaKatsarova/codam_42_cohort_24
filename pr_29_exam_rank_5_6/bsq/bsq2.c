...
// 9 . 0 x [<num of lines> <empty char> <obstacle> <full char>]
  ...
  int read_map(FILE *fp, int n, char empty_c, char obst_c, char full_c,
               char ***map_out, int *width_out)
  {
      ...
      while (i < n)
      {
          ...
          while (j < width)
          {
              if (line[j] != empty_c && line[j] != obst_c && line[j] != full_c)
              {
                  free(line);
                  free_map(map, i);
                  return (-1);
  ...
  */
  ...
  int process_map(FILE *fp)
  {
      ...
      if (fscanf(fp, "%d %c %c %c", &n, &empty_c, &obst_c, &full_c) != 4)
          return (-1);
      if (empty_c == obst_c || empty_c == full_c || obst_c == full_c)
          return (-1);
      if (n < 1)
          return (-1);
      junk = NULL;
      ...
      free(junk);
      if (read_map(fp, n, empty_c, obst_c, full_c, &map, &width) == -1)
          return (-1);
