...
static int	smallest_island(const t_grid *grid, char *seen, int *stack)
{
	int	idx;
	int	total;
	int	best;
	...
	while (idx < total)
	{
		...
		{
			size = flood_size(grid, seen, idx, stack);
			if (has_one == 0 || size < best)
				best = size;
			has_one = 1;
		}
		idx++;
...
int	main(int argc, char **argv)
{
	...
	stack = (int *)malloc((size_t)(cells * (int)sizeof(int)));
	...
	result = smallest_island(&grid, seen, stack);
	put_nbr(result);
	write(1, "\n", 1);
	free(seen);
	free(stack);
