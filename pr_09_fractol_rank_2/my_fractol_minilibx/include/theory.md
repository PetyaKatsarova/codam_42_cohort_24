A fractal is a mathematical pattern that repeats itself at different scales. It is self-similar, meaning that zooming in reveals smaller versions of the whole shape. Fractals are often generated using recursive algorithms.

Each point (x, y) on the screen corresponds to a complex number (cx, cy), and we apply iterative functions to determine if the point belongs to the fractal.

cx	Complex real part (c)	cx = x / zoom + offset_x
cy	Complex imaginary part (c)	cy = y / zoom + offset_y
zx	Real part of z_n	Changes in each iteration
zy	Imaginary part of z_n	Changes in each iteration

Mandelbrot Set

    We start with zx = 0 and zy = 0.
    cx and cy are taken from the pixel’s (x, y) position.
    The formula applied iteratively:
    zn+1=zn2+c
    zn+1​=zn2​+c Expanded:
    zxnew=zx2−zy2+cx
    zxnew​=zx2−zy2+cx
    zynew=2⋅zx⋅zy+cy
    zynew​=2⋅zx⋅zy+cy
        If |z| grows too large, the point escapes the Mandelbrot set.

 Julia Set

    cx and cy are fixed constants (determining shape).
    zx and zy start as the pixel’s (x, y).
    The iteration formula is the same as Mandelbrot:
    zxnew=zx2−zy2+cx
    zxnew​=zx2−zy2+cx
    zynew=2⋅zx⋅zy+cy
    zynew​=2⋅zx⋅zy+cy
        Instead of checking if c belongs to the set, we check if z escapes.

cx, cy: The complex number (c) defining the fractal point.
zx, zy: The iterative values determining escape.
Formula:
zxnew=zx2−zy2+cx
zxnew​=zx2−zy2+cx
zynew=2⋅zx⋅zy+cy
...
(zx+zyi)(zx+zyi)=(zx*zx−zy*zy)+(2*zx*zy)(-1) // sqrt i = -1
